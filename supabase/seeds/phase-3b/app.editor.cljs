...
(defn- save!
  "Run the save flow. Returns a Promise resolving to a map:
     {:version  new-version-number
      :impact   [affected-ns ...]}
   Throws on failure."
  [ns-name draft-source prior-version deps]
  ...
  (let [!draft (r/atom (:source initial-row))
        ...
        on-save (fn []
                  (when-not @!saving?
                    (reset! !saving? true)
                    (reset! !error nil)
                    (-> (save! ns-name @!draft @!version @!deps)
                        (.then (fn [r]
